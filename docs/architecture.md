...
Two WebSocket channels share the same port. **Socket.IO** (mounted on `/socket.io`) carries meeting commands such as starting the meeting, granting the floor, raising a hand or moving topics, as well as the periodic `meeting:state` broadcasts that drive the entire UI. **Yjs** (mounted on `/yjs/<meetingId>?token=…`) carries the CRDT synchronization for the collaborative Markdown notes plus the awareness payload that produces the colored remote cursors. There is no separate persistence layer: everything lives in server memory, and a meeting is garbage-collected after `HOST_TIMEOUT_MS` of inactivity or after `POST_END_GC_MS` once it has been ended explicitly.

## Workspaces

The repo is a three-workspace npm monorepo. `shared/` holds the TypeScript contracts (the `Meeting` shape and the `ClientToServerEvents` / `ServerToClientEvents` signatures). It has no runtime dependencies so it can be imported from either Node or the browser. `server/` is the Express 5 backend plus the Socket.IO server and the Yjs bridge; it compiles to `server/dist/`. `client/` is a Vite 8 + vanilla TypeScript frontend that bundles to `server/dist/public/`, which the server then serves as static assets in production.

## Server

`meetings/Meeting.ts` is the state machine for one meeting. All transitions (start, pause, end, grant or revoke speaker, raise or lower hand, add or remove topics, promote or demote a host, reorder participants or topics, toggle the time-box) live as methods on the class. The socket handlers must never mutate `meeting.state` directly; they call a method and rebroadcast. The per-speaker and per-topic accumulators (`flushSpeaker`, `flushTopic`) run on every transition that pauses or hands off timing, and that bookkeeping only lives inside the class methods so it cannot be bypassed. A monotonic counter `orderSeq` assigns a unique `order` value to each participant: two participants created in the same millisecond would otherwise collide on `Date.now()` alone and the up/down reorder would silently fail. `grantSpeaker` and `setCurrentTopic` are no-ops outside the `running` or `paused` phases, and `start()` is a no-op once the meeting has ended. Because a pause re-arms `currentSpeakerStartedAt`, `flushSpeaker` also banks the elapsed time into `currentSpeakerTurnMs`, which a new grant resets, so the client can show the current turn's time (and enforce the time-box) across a pause. Client-supplied participant ids go through `participant(id)`, an own-property lookup. `end()` flushes the accumulators, clears `currentSpeakerId` and `currentTopicId` (so the UI treats `ended` as fully idle) and records `endedAt`. The optional password is stored privately on the instance; only `state.hasPassword` is broadcast, and verification goes through `verifyPassword(supplied, ip)` using a constant-time-ish comparison (length check first, then per-character XOR).

`meetings/MeetingStore.ts` is the in-memory map of active meetings and their Yjs documents. A periodic GC sweep deletes meetings with no connected participant whose `lastActivity` is older than `HOST_TIMEOUT_MS`; presence counts as activity, so only a meeting that stays empty for the whole window is collected and a momentary full disconnect never deletes a live meeting. The store also exposes `scheduleDeleteAfterEnd(id, delayMs)`, called by the `meeting:end` handler so a terminated meeting (with its tokens, password, participants, topics and Y.Doc) is wiped from memory after `POST_END_GC_MS` (default five minutes). The delay leaves enough time for the client to export the Markdown notes from the still-live Yjs document.

`meetings/hostFallback.ts` is the safety net that auto-promotes the oldest connected participant when no host remains, so an accidental disconnect of every host does not lock the meeting. The promotion waits `HOST_FALLBACK_GRACE_MS` (fifteen seconds) after the last host disconnects, so a host who reloads the page or rides out a short network drop comes back still in charge; `Meeting` records when it became hostless, and the socket layer re-checks once the grace period is over. Removing or demoting a host still promotes a fallback at once.

`socket/handlers.ts` wires each Socket.IO event to a `Meeting` method and rebroadcasts the state; `meeting:state` is the only server-to-client event, and clients derive every UI change by diffing successive states. Every listener is registered through a guard that catches exceptions from malformed frames (missing payload, non-function ack) and answers the ack with `invalid_payload` instead of letting the exception take the process down. `socket/authorize.ts` carries the `requireHost` guard that gates every host-sensitive event on the participant's `isHost` flag. The exceptions are `hand:raise`/`hand:lower` and the participant-driven `speaker:claim`/`speaker:release`: a participant takes or releases the floor for themselves (reusing `grantSpeaker`/`revokeSpeaker`, so the host can still override). Presence is multi-socket aware: a participant connected from two devices stays connected until their last socket drops, and an identity-flow join whose first name, last name and role match an existing participant reuses that participant instead of creating a duplicate (so a laptop plus a phone is one identity, not two). `yjs/ywsBridge.ts` is the minimal y-websocket-compatible bridge that drops doc updates coming from non-hosts so guests cannot tamper with the notes.
...
The client has no UI framework. Components are functions that return a handle of the shape `{ el: HTMLElement; update(): void; tick?(): void; destroy?(): void }`. `pages/MeetingPage.ts` is canonical: it builds every component once, subscribes to `meeting$` to call `update()` on state changes, and runs a single five-hundred-millisecond ticker that calls `meetingTimer.tick()`, `spotlight.update()`, `list.tick()` and `agenda.tick()`. The single ticker means every component reads the same `Date.now()` value, so the spotlight chronometer and the participant-row chronometer cannot drift apart by a second.
...
`i18n/index.ts` exposes `t(key, params)` with a fallback chain (current locale → French → raw key), `setLocale`, the `locale$` observable and the canonical `SUPPORTED_LOCALES` array used by the picker. Dictionaries live in `client/src/i18n/locales/<locale>.json`, one file per supported locale with `fr` as the source of truth (see `i18n.md` for the current list).

`components/Avatar.ts` is the round avatar with initials, a deterministic color (`lib/color.ts`, darkened per hue until the white initials clear 4.5:1) and a status badge (speaking or hand-raised). It carries `role="img"` plus a reconstructed `aria-label` (full name, role, connection or status).
...
`components/ParticipantList.ts` is the participant cards. Each row carries a fill bar proportional to that participant's share of the total speaking time (deterministic color plus a stripe pattern whose angle varies by index, so two close hues stay distinguishable for color-blind viewers). Reorder is handled by chevron buttons and by native HTML5 drag-and-drop, the row being marked `draggable=true` only while the user holds the dedicated drag handle, so inner button clicks never start an accidental drag. The leading speaker control (grant or revoke) and the host actions (promote, demote, reorder, remove) are gated on `phase !== "ended"`.

`components/HandRaiseBanner.ts` is the full-width banner that surfaces a queue of raised hands. The first hand is highlighted; subsequent hands appear as chips. While the meeting is running or paused, hosts can click a chip to grant the floor directly to that participant; in the lobby and after the end the banner has no grant controls.
...
`components/MobileMeetingView.ts` is the phone-sized participant view rendered below the breakpoint. It shows the meeting timer, the current speaker and the topic under discussion, with a large take/release-the-floor button (whose label switches to "take over from {name}" when a colleague holds the floor), a raise-hand button, and sound and vibration toggles, and it holds a Screen Wake Lock (released once the meeting ends) so the phone does not dim mid-meeting. It reuses `SpeakerSpotlight` (with the 10..1 s countdown cues scoped to the current speaker's own device) and `MeetingTimer` and deliberately omits the agenda, participant list and notes editor. This is a design decision, not a gap: the mobile view is a comfort layer for taking and releasing the floor and raising a hand; the desktop dashboard is the primary interface and running a meeting requires a large screen. A host who opens the app on a phone (or a portrait tablet below 900px) gets the participant view with no meeting controls, deliberately. Do not add host controls to this view.

`components/MarkdownPreview.ts` renders Markdown through Marked, with syntax highlighting from a lazy-loaded Shiki highlighter (dual themes `github-light` and `github-dark`, fourteen bundled languages). The output is sanitized by DOMPurify with `ADD_ATTR: ["style"]` so Shiki's inline styles are preserved; a DOMPurify hook keeps a `style` attribute only when it holds `--shiki-*` custom properties.

`components/MarkdownHelpDialog.ts`, `AddTopicDialog.ts`, `AddParticipantDialog.ts`, `ConfirmDialog.ts`, `KeyboardHelpDialog.ts` and `ShareMeetingDialog.ts` are the modal dialogs (backdrop plus dialog plus actions, Escape and click-outside to close). `confirmDialog` takes a mandatory `okLabel` that repeats the action ("End the meeting", never a bare "Confirm") and a `danger` flag that paints it red for irreversible actions. `ShareMeetingDialog` shows the direct join link (`#/join?id=…`, with the password deliberately kept out of the URL since URLs leak via history and the Referer header), the meeting ID, the password (only if known locally), and a ready-to-paste invitation message. It is also reachable from the meeting header via a Share button.
...
`lib/meetingImport.ts` parses, serializes and downloads JSON meeting templates. See `meeting_import.md`.
...
`lib/sounds.ts` synthesizes sounds through the Web Audio API. No binary audio asset ships.
...
`lib/color.ts` exposes `colorByPosition(idx, total, salt)` which produces visually distinct colors deterministically for one meeting.

## Event flow (example: raising a hand)

1. The participant clicks the hand button; the client emits `hand:raise`.
2. `socket/handlers.ts` receives the event, calls `meeting.raiseHand(participantId)` and broadcasts `meeting:state`.
