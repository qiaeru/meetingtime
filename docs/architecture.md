...
`components/MarkdownPreview.ts` renders Markdown through Marked, with syntax highlighting from a lazy-loaded Shiki highlighter (dual themes `github-light` and `github-dark`, fifteen bundled languages). The output is sanitized by DOMPurify with `ADD_ATTR: ["style"]` so Shiki's inline styles are preserved.
