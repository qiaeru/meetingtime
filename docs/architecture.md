...
`components/Avatar.ts` is the round avatar with initials, a deterministic color and a status badge (speaking or hand-raised). It carries `role="img"` plus a reconstructed `aria-label` (full name, role, connection or status).
