...
- `TRUST_PROXY=1` is set automatically so Express honors the `X-Forwarded-*` headers Caddy sets. WebSocket upgrades for Socket.IO and the Yjs notes channel need no extra configuration: Caddy's `reverse_proxy` passes them through natively.
...
- To update the stack: `git pull && docker compose -f deploy/caddy/docker-compose.caddy.yml up -d --build`.
